"""
OpenAI Vision Provider using GPT-4 Vision (gpt-4o) via LangChain.

This provider uses OpenAI's vision capabilities to extract and translate
...
def _get_client() -> Optional[ChatOpenAI]:
    """Get or create OpenAI client instance (thread-safe lazy initialization)."""
    global _client
...
class OpenAIVisionProvider(VisionProvider):
    """
    Vision provider implementation using OpenAI GPT-4 Vision API via LangChain.

    This provider uses OpenAI's vision capabilities to:
    ...
    def _parse_response(self, response_text: str) -> List[TextExtraction]:
        ...
        if "NO_TEXT_FOUND" in response_text:
            logger.info("No text found in image (OpenAI)")
            return []
    ...
    async def extract_text(
        ...
        """
        Extract and translate text from image using OpenAI Vision API.
        ...
        try:
            ...
            # Call OpenAI API (run in thread to avoid blocking)
            logger.info("Calling OpenAI Vision API", model=config.OPENAI_VISION_MODEL)
            ...
            raise VisionProviderError(
                f"OpenAI vision extraction failed: {str(e)}",
                provider=self.name
            ) from e
